...
Function RunLog-Command([string]$Description, [ScriptBlock]$Command, [string]$Color){
...
Function WriteLog([string]$Description, [string]$Color){
    If (!($Color)) {$Color="Yellow"}
    $Output = $Description+'  ... '
    Write-Host $Output -ForegroundColor $Color
    ((Get-Date -UFormat "[%d-%m-%Y %H:%M:%S] ") + $Output) | Out-File -FilePath $LogFile -Append -Force   
...
function DownloadWithRetry
{
    ...
    while($true)
    {
        ...
        catch
        {
            ...
            WriteLog -Message "Error downloading '$url': $exceptionMessage"
            if ($retries -gt 0) {
                $retries--
                WriteLog -Message ("Waiting 10 seconds before retrying. Retries left: " + $retries)
                Start-Sleep -Seconds 10
 
            }else{
                $exception = $_.Exception
                WriteLog -Message "Failed to download '$url': $exceptionMessage"
                break
            }
        }
    }
